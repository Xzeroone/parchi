import type { ErrorCategory } from './classifier.js';
...
export function detectManagedRouteSignal(context: ManagedSignalInput, combined: string): boolean {
  ...
  return (
    ...
    combined.includes('billing is not active') ||
    combined.includes('subscription is not active') ||
    combined.includes('start or manage your stripe plan')
  );
}
...
export function classifyManagedRuntimeSpecialCase(
  ...
  if (
    statusCode === 402 ||
    combined.includes('billing is not active') ||
    combined.includes('start billing') ||
    combined.includes('subscription is not active') ||
    combined.includes('stripe plan')
  ) {
    return {
      category: 'auth',
      message: 'Managed access is unavailable for this request.',
      action: 'Open Account & Billing to start or reactivate your Stripe billing plan, then retry.',
      recoverable: true,
    };
  }
