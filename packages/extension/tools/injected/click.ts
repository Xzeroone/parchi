...
export type InjectedClickResult =
  | { success: true; strategy: string; candidates: number }
  | { success: false; error: string; hint?: string; strategy?: string; candidates?: number };
...
export const injectedClick = async (spec: SelectorSpec, waitMs: number): Promise<InjectedClickResult> => {
  ...
  // Inline dispatchSyntheticClick to avoid new Function reconstruction on CSP-strict pages
  const dispatchSyntheticClick = (
    ...
    const fireMouse = (type: string) => {
      target.dispatchEvent(
        new MouseEvent(type, { bubbles: true, cancelable: true, view: window, clientX, clientY, button: buttonCode }),
      );
    };
    ...
    if (options.contextMenu) fireMouse('contextmenu');
    const clickable = target as Element & { click?: () => void };
    if (buttonCode === 0 && typeof clickable.click === 'function') clickable.click();
  };
  ...
  const resolved = resolveSelectorSpecElement(spec as any, true);
  if (resolved.el) {
    const result = clickElement(resolved.el);
    return { ...result, strategy: resolved.strategy, candidates: resolved.candidates };
  }
