...
export async function runThinkingExtractionSuite(runner: TestRunner) {
  log('\n=== Testing Thinking Extraction ===', 'info');

  await runner.test('extractThinking strips <analysis> tags', () => {
  ...
  await runner.test('extractThinking merges think + analysis with existing notes', () => {
