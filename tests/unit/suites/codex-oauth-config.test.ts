...
export async function runCodexOauthConfigSuite(runner: TestRunner) {
  log('\n=== Testing Codex OAuth Runtime Config ===', 'info');

  await runner.test('Codex OAuth provider detection works', () => {
    runner.assertTrue(isCodexOAuthProvider('codex-oauth'));
    runner.assertFalse(isCodexOAuthProvider('openai'));
  });

  await runner.test('Codex OAuth provider options force store=false and instructions', () => {
  ...
  await runner.test('Codex OAuth base URLs target ChatGPT codex endpoint', () => {
