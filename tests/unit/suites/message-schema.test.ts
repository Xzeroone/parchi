...
export async function runMessageSchemaSuite(runner: TestRunner) {
  log('\n=== Testing Message Schema ===', 'info');

  await runner.test('createMessage builds canonical message', () => {
  ...
  await runner.test('normalizeConversationHistory filters invalid messages', () => {
  ...
  await runner.test('toProviderMessages serializes tool calls and results', () => {
  ...
  await runner.test('thinking metadata is preserved and not sent to provider', () => {
  ...
  await runner.test(
    'normalizeConversationHistory hydrates tool metadata and respects addIds/addTimestamps flags',
    () => {
      const normalized = normalizeConversationHistory(
        [
          {
  ...
  await runner.test('toProviderMessages normalizes array content and serializes tool payloads', () => {
