...
export function runModelCapabilitiesSuite(runner: TestRunner) {
  log('\n=== Testing Model Capabilities ===', 'info');

  runner.test('resolveModelCapabilities treats Anthropic and Claude OAuth as always vision-capable', () => {
  ...
  runner.test('resolveModelCapabilities treats every Kimi model as vision-capable', () => {
  ...
  runner.test('resolveModelCapabilities matches GLM vision variants only', () => {
  ...
  runner.test('resolveModelCapabilities matches MiniMax vision variants only', () => {
  ...
  runner.test('resolveModelCapabilities matches Codex/Copilot OAuth gpt-4o and vision models', () => {
  ...
  runner.test('resolveModelCapabilities matches OpenRouter/Parchi known vision-capable model families', () => {
  ...
  runner.test('resolveModelCapabilities matches OpenAI gpt-4o/4.1/vision model families', () => {
  ...
  runner.test('resolveModelCapabilities matches legacy google provider gemini/imagen models', () => {
  ...
  runner.test('resolveModelCapabilities falls back to a bare "vision" match for unknown providers', () => {
  ...
  runner.test('resolveModelCapabilities returns false with no provider', () => {
    runner.assertFalse(resolveModelCapabilities('', 'gpt-4o').supportsVision);
  });

  runner.test('resolveModelCapabilities prefers an explicit static ModelEntry.supportsVision flag', () => {
  ...
  runner.test('isVisionModelProfile preserves prior behavior for every previously-handled provider', () => {
  ...
  runner.test('extractModelEntries backfills supportsVision via capability rules when given a provider key', () => {
  ...
  runner.test('extractModelEntries does not compute supportsVision when no provider key is given', () => {
